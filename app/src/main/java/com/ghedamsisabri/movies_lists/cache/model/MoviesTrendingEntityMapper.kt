...
class MoviesTrendingEntityMapper: DomainMapper<MoviesTradingEntity, Movies> {
    override fun mapToDomainModel(model: MoviesTradingEntity): Movies {
        return(Movies(
            id = model.id,
            adult=model.adult,
    ...
    override fun mapFromDomainModel(domainModel: Movies): MoviesTradingEntity {
        return(MoviesTradingEntity(
            id = domainModel.id,
    ...
    fun fromEntityList(initial: List<MoviesTradingEntity>): List<Movies>{
        return initial.map { mapToDomainModel(it) }
    }
    fun fromEntityList(initial: MoviesTradingEntity): Movies{
        return mapToDomainModel(initial)
    }
    fun toEntityList(initial: List<Movies>): List<MoviesTradingEntity>{
