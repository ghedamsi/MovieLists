...
class MoviesUpcomingEntityMapper: DomainMapper<MovieUpcomingEntity, Movies> {
    override fun mapToDomainModel(model: MovieUpcomingEntity): Movies {
        return(Movies(
            id = model.id,
            adult=model.adult,
    ...
    override fun mapFromDomainModel(domainModel: Movies): MovieUpcomingEntity {
    ...
    fun fromEntityList(initial: List<MovieUpcomingEntity>): List<Movies>{
        return initial.map { mapToDomainModel(it) }
    }
    fun fromEntityList(initial:MovieUpcomingEntity):Movies{
        return mapToDomainModel(initial)
    }
    fun fromEntityList(initial:Movies):MovieUpcomingEntity{
        return mapFromDomainModel(initial)
    }


    fun toEntityList(initial: List<Movies>): List<MovieUpcomingEntity>{
